	...
	try {
		...
		if (db.users[m.sender].money >= count * 1) {
			...
			if (Aku > Kamu) {
				...
				m.reply(`💰 Casino 💰\n*You:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nYou got ${count * 2} money`.trim())
			} else {
				db.users[m.sender].money += count * 1
				m.reply(`💰 Casino 💰\n*You:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*SERI*\nYou got ${count * 1} Money`.trim())
			}
		} else m.reply(`You don't have enough money for the casino, please collect it first!`)
	} catch (e) {
		m.reply('Error!')
	}
}
...
const gameSamgongSolo = async (conn, m, db) => {
	...
	if (isNaN(count) || count < 5000) return m.reply('The minimum bet is5000!');
	if (db.users[m.sender].money < count) return m.reply(`You don't have enough money for the casino, please collect it first!`)
	db.users[m.sender].money -= count;
	db.users[m.sender].limit -= 1
	let { key } = await m.reply('*🃏The game has started! Cards are being dealt...');
	await sleep(5000);
